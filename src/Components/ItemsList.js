  ...
  if (
    ...
    return tasksToDo.map((element, index) => {
      if (element.done === false) {
        return (
          <div className="flex gap-1 bg-white px-6 py-3 rounded" key={index}>
            <p className="w-full text-teal-800 truncate">{element.title}</p>
            <button
              className="bg-teal-400 hover:bg-teal-300 rounded-full p-1"
              onClick={() => props.handleCheckTask(index, true)}
            >
              <svg
